...
/// <summary>
/// A flexible HTTP API mock server for testing.
/// </summary>
/// <remarks>
/// MockServer allows you to create mock HTTP endpoints with customizable responses.
...
public class MockServer : IAsyncDisposable
{
    ...
    private MockWebHost? webHost;
    private bool disposed;

    /// <summary>
    /// Initializes a new instance of the <see cref="MockServer"/> class.
    /// </summary>
    /// <param name="options">Configuration options for the mock server.</param>
    public MockServer(MockServerOptions? options = null)
    {
        this.options = options ?? new MockServerOptions();
    }

    /// <summary>
    /// Gets a value indicating whether the mock server is currently running.
    /// </summary>
    ...
    {
        if (webHost != null)
            throw new InvalidOperationException("MockServer is already started. Call StopAsync() before starting again.");

        webHost = new MockWebHost(options, this);
        await webHost.StartAsync(cancellationToken);
    }

    /// <summary>
    /// Stops the mock server asynchronously.
    /// </summary>
    /// <param name="cancellationToken">Cancellation token for async operations.</param>
    ...
    /// <summary>
    /// Creates an HttpClient configured to communicate with the mock server.
    /// </summary>
    /// <returns>A new HttpClient instance.</returns>
    /// <exception cref="InvalidOperationException">Thrown if the server is not started.</exception>
    public HttpClient CreateClient()
    {
        if (webHost == null)
            throw new InvalidOperationException("MockServer must be started before creating clients. Call StartAsync() first.");
    ...
    /// <summary>
    /// Asynchronously disposes of the mock server and releases all resources.
    /// </summary>
    public async ValueTask DisposeAsync()
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine($"Error during MockServer disposal: {ex}");
        }
